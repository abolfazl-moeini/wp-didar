...
function did_paging( &$query, $cnt = 10 ) {
    ...
    <ul class="pagination justify-content-center mb-0">
        <li class="page-item"><input type="submit" class="page-link" name="paging[<?php echo '0'; ?>]" class="btn"
                                     value="«" <?php if ( absint( $page ) == 0 )
                echo "disabled" ?>/></li>
        <li class="page-item"><input type="submit" class="page-link" name="paging[<?php if ( absint( $page ) == 0 ) {
                echo "0";
            } else {
                echo absint( $page ) - 1;
            } ?>]" class="btn" value="‹" <?php if ( absint( $page ) == 0 )
                echo "disabled" ?>/></li>
        ...
        $arr = array_merge( range( 0, ( $page < $rng ? $page : $rng - 1 ) ), range( $mi1, $mi2 ),
                range( $mi3, $max_page ) );
        ...
        <li class="page-item"><input type="submit" class="page-link"
                                     name="paging[<?php if ( absint( $page ) < absint( $max_page ) ) {
                                         echo absint( $page ) + 1;
                                     } else {
                                         echo absint( $max_page );
                                     } ?>]" class="btn" value="›" <?php if ( absint( absint( $page ) ) == $max_page )
                echo "disabled" ?>/></li>
        <li class="page-item"><input type="submit" class="page-link" name="paging[<?php echo absint( $max_page ); ?>]"
                                     class="btn" value="»" <?php if ( absint( $page ) == absint( $max_page ) )
                echo "disabled" ?>/></li>
