<div class="wrap">
    ...
    <p><?php _e( '1.Reset webservice and start send order info from begin order if you leave input empty or  zero',
                'didar' ); ?></p>
    ...
    if ( isset( $_POST['reset'] ) ) {
        ...
        echo '<div class="notice notice-success is-dismissible"><p>' . __( 'didar webservice reset successfull.',
                        'didar' ) . '</p></div>';
    }
    ...
    <form method="post">
        <fieldset>
            ...
            <button type="submit" name="reset" class="button"
                    onclick="confirm('<?php _e( 'Are you sure reset webservice?', 'didar' ); ?>')"><?php _e( 'Reset',
                        'didar' ); ?></button>
        </fieldset>
    </form>
</div>
